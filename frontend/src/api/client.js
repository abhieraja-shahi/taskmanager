...
api.interceptors.response.use(
  ...
  (err) => {
    if (err.response?.status === 401) {
      ...
      return new Promise(() => {}) // prevent component catch handlers from firing during redirect
    }
...
export const reopenTask = (id, data) => api.post(`/tasks/${id}/reopen`, data)
...
/* ─── Zammad Tickets ─────────────────────────────────────────────────── */
export const getZammadTickets    = (params)   => api.get('/zammad/tickets', { params })  // params: { state, page, page_size }
